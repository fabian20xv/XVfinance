 ...
 * Schema migrations remain pinned to the parent/prod project.
 */
...
if (!/^SUPABASE_JWT_SECRET=/m.test(envExample)) {
  fail('.env.example must document SUPABASE_JWT_SECRET');
} else {
  ok('.env.example documents SUPABASE_JWT_SECRET');
}

if (!/^PORT=8787$/m.test(envExample)) {
  fail('.env.example must keep PORT=8787 for the E0 HTTP API');
} else {
  ok('.env.example documents PORT=8787');
}
...
for (const name of readdirSync(migrationsDir).filter((file) => file.endsWith('.sql'))) {
  ...
  if (!sql.includes(PARENT_REF)) {
    fail(`${name} must name the parent/prod project ref`);
  } else {
    ok(`${name} is pinned to ${PARENT_REF}`);
  }
}
