...
| SpeakReadyToggle | Speak 1.5, mounted in the top bar only while the meeting workspace is open. Speak-ready adds `is-speak-ready` and `meeting-one-pager--speak` (body 18/28 via `--speak-body-size` / `--speak-body-line`) and dims top-bar chrome with `speak-ready-dim` at `--speak-chrome-opacity` (0.4, ink-muted). `R` opens the first receipt side-slip (no-op if one is already open; ignored while typing). Toggle off or Leave restores the prior layout. Client-only — no `/v1` speak route, no voice or mic |
